...
if conn.is_connected():
    ...
    print('<thead> <tr> <th scope="col">Action</th><th scope="col">id</th><th scope="col">DepartmentName</th><th scope="col">Description</th></tr> </thead>') 
    
    print('<tbody> ')
    for x in result:
        print(f'<tr><td><a href="Editdepartment.py?id={x[0]}"><input type="button" value="Edit"><br><br><a href="departmentdelete.py?id={x[0]}"><input type="button" value="Delete"></a></td> <td scope="row">{x[0]}</td> <td>{x[1]}</td> <td>{x[2]}</td> ')
        print('</tr>')
