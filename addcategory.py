...
if conn.is_connected():
    ...
    query="INSERT INTO addcategory(id,Categoryname,Description,GST) VALUES (%s, %s, %s, %s)"
    val=(id,Categoryname,Description,GST)
    ...
    print('<thead> <tr> <th scope="col">id</th><th scope="col">Categoryname</th><th scope="col">Description</th><th scope="col">GST</th> </tr> </thead>') 
    
    print('<tbody> ')
    for x in result:
        print(f'<tr> <td scope="row">{x[0]}</td> <td>{x[1]}</td> <td>{x[2]}</td> <td>{x[3]}</td>')
        print('</tr>')
