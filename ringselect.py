...
if conn.is_connected():
    ...
    print('<thead> <tr><th>Delete</th><th>Edit</th> <th scope="col">id</th><th scope="col">Jewelleyname</th><th scope="col">Category</th><th scope="col">Weight</th><th scope="col">Quantity</th><th scope="col">Description</th><th scope="col">GST</th><th scope="col">Totalprice</th><th scope="col">Stock</th>  </tr> </thead>') 
    
    print('<tbody> ')
    for x in result:
        print(f'<tr><td><a href=delete.py >Delete</a> <td><a href=edit.py?id={x[0]}&Jwelleryname={x[1]}&Category={x[2]}&Weight={x[3]}&Quantity={x[4]}&Description={x[5]}&GST={x[6]}&Totalprice={x[7]}&Stock={x[8]}>Edit</a></td><td scope="row">{x[0]}</td><td>{x[1]}</td><td>{x[2]}</td><td>{x[3]}</td><td>{x[4]}</td><td>{x[5]}</td><td>{x[6]}</td><td>{x[7]}</td><td>{x[8]}</td> ')
        print('</tr>')
