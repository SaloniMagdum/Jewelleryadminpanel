...
if conn.is_connected():
    ...
    print('<thead> <tr><th scope="col">Action</th> <th scope="col">id</th><th scope="col">Name</th><th scope="col">Phone</th><th scope="col">Address</th><th scope="col">AdharNo</th><th scope="col">PanNo</th><th scope="col">Department</th><th scope="col">AccountNo</th>  </tr> </thead>') 
    
    print('<tbody> ')
    for x in result:
        print(f'<tr><td><a href="Editemployee.py?id={x[0]}"><input type="button" value="Edit"><br><br><a href="employeedelete.py?id={x[0]}"><input type="button" value="Delete"></a></td> <td scope="row">{x[0]}</td> <td>{x[1]}</td> <td>{x[2]}</td> <td>{x[3]}</td> <td>{x[4]}</td> <td>{x[5]}</td> <td>{x[6]}</td> <td>{x[7]}</td>')
        print('</tr>')
